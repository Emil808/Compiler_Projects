...
EnumSet<PascalTokenType> CaseStatementParser::CONSTANT_START_SET =
{
    PT_IDENTIFIER, PT_INTEGER, PT_PLUS, PT_MINUS, PT_STRING,
};

EnumSet<PascalTokenType> CaseStatementParser::OF_SET;
EnumSet<PascalTokenType> CaseStatementParser::COMMA_SET;
    ...
    throw (string)
{
    ...
    if (constant_node != nullptr)
    {
        Object *data_value = constant_node->get_value();
    ...
    throw (string)
{
    ...
    {
        Object *constant_value = constant_id->get_data_value();
        ...
        constant_node->set_value(*constant_value);
    }
                              ...
                              const string value, const bool minus_sign)
{
    ...
    constant_node->set_value(int_value);
    return constant_node;
}
    ...
    else
    {
        if (value.length() == 1)
        {
            constant_node =
                ICodeFactory::create_icode_node(
                                     (ICodeNodeType) NT_INTEGER_CONSTANT);
            constant_node->set_value((int) value[0]);
        }
        else
        {
            error_handler.flag(token, INVALID_CONSTANT, this);
